...
# === End: Save VGGT preprocessed network input images ===
    ...
    # <<<< ADDED: Save depth, intrinsics, and pose for SAM3D START >>>>
     print("DEBUG: extrinsics.shape =", extrinsics.shape)
    print("DEBUG: intrinsics.shape =", intrinsics.shape)
    print("DEBUG: depth_maps.shape =", depth_maps.shape)

    # 2) 准备输出文件夹
    out_base   = os.path.join(args.scene_dir, "images")      # 指向 …/images
    ...
    # 3) 按 batch 保存
    for i, fname in enumerate(base_image_path_list):
        base = os.path.splitext(fname)[0]
        # 深度：米 -> 毫米，uint16 PNG
        d = depth_maps[i]                      # ← 注意，这里用 depth_maps
        cv2.imwrite(
            os.path.join(depth_dir, f"{base}.png"),
            (d * 1000.0).astype(np.uint16)
        )
        # 内参：3x3 矩阵
        np.savetxt(
            os.path.join(intri_dir, f"{base}.txt"),
            intrinsics[i],
            fmt="%.6f"
        )
        # Pose：取 4×4 的前 3 行
        ...
        np.savetxt(
            os.path.join(pose_dir, f"{base}.txt"),
            P3,
            fmt="%.6f"
        )
